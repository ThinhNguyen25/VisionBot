...
const uint8_t MQTT_AUTO_PORTAL_FAIL_COUNT = 3;
...
bool runConfigPortal(bool forcePortal, const String &reason) {
  ...
  Serial.println("If MQTT fails 3 times consecutively, config portal opens automatically too.");
...
void recordMqttFailureAndMaybeOpenPortal() {
  mqttConsecutiveFailures++;

  Serial.print("MQTT consecutive failures: ");
  Serial.print(mqttConsecutiveFailures);
  Serial.print("/");
  Serial.println(MQTT_AUTO_PORTAL_FAIL_COUNT);

  if (mqttConsecutiveFailures >= MQTT_AUTO_PORTAL_FAIL_COUNT) {
    openConfigPortalForRecovery("mqtt_connect_failed_3_times");
  }
}
